...
class activity_criar_categoria_receita : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_criar_categoria_receita)
    ...
    private fun salvarCategoriaNoFirestore(nomeCategoria: String, corCategoria: Int) {
        ...
        val categoriaData = hashMapOf(
            "categoria_criada_pelo_usuario_receita" to nomeCategoria,
            "cor_criada_pelo_usuario_receita" to corCategoria,
            "usuario_da_categoria_receita" to userId
        )
        ...
        db.collection("categorias")  // Substitua pelo nome da sua coleção no Firestore
            ...
            .addOnSuccessListener { documentReference ->
                Log.d(ContentValues.TAG, "Categoria criada com sucesso!")
    ...
    private fun verificarExistenciaCategoria(nomeCategoria: String) {
        ...
        Log.d(ContentValues.TAG, "Verificando existência da categoria: $nomeCategoria")

        val nomeCategoriaSemEspacos = nomeCategoria.trim()

        // Função para verificar a existência da categoria em "categorias_despesas"
        db.collection("categorias").document("categoria_receitas")
            .get()
            .addOnSuccessListener { documentSnapshotReceitas->
                if (documentSnapshotReceitas.exists()) {
                    val categoriasReceitas = documentSnapshotReceitas.data

                    // Verifica se a categoria fornecida está entre as categorias predefinidas
                    if (categoriasReceitas != null && categoriasReceitas.containsValue(nomeCategoriaSemEspacos)) {
                        ...
                        db.collection("categorias")
                            .whereEqualTo("categoria_criada_pelo_usuario_receita", nomeCategoriaSemEspacos)
                            ...
                            .addOnFailureListener { e ->
                                Log.w(ContentValues.TAG, "Erro ao verificar categoria de usuário", e)
                            }
                    }
                } else {
                    Log.d(ContentValues.TAG, "Documento 'categoria_receitas' não existe")
                }
            }
            .addOnFailureListener { e ->
                Log.w(ContentValues.TAG, "Erro ao verificar categoria de receitas", e)
            }
    }
}